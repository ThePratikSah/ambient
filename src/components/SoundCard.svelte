<script lang="ts">
  ...
  let { sound, state }: { sound: Sound; state: SoundState } = $props();
...
<button
  ...
  class:ring-2={state.playing}
  class:ring-blue-400={state.playing}
  class:dark:ring-blue-500={state.playing}
>
  {#if state.playing}
    <span class="absolute top-3 right-3 w-2.5 h-2.5 rounded-full bg-blue-500 animate-pulse"></span>
  {/if}

  <div class="flex flex-col items-center gap-3 mt-2">
    <div class="w-12 h-12 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 text-xl select-none">
      {sound.title.charAt(0)}
    </div>
  ...
  <div class="w-full mt-4 px-1" onclick={(e: Event) => e.stopPropagation()} onkeydown={() => {}}>
    <input
      type="range"
      min="0"
      max="100"
